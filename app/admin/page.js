  ...
  if (!authorized) {
    return (
      <div style={{ maxWidth: 360, margin: '80px auto', padding: 24, background: '#fff', border: '1px solid #e0e0e0', borderRadius: 8 }}>
        ...
        <p style={{ fontSize: 11, color: '#999', marginTop: 14, textAlign: 'center' }}>
          أول مرة؟ جرّب <b>admin</b> / <b>1234</b> (لو محدّتش الحساب من قبل كده)
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '30px auto', padding: '0 16px 60px' }}>
      ...
      {!loading && devices.length === 0 && (
        <p style={{ fontSize: 13, color: '#666' }}>مفيش أجهزة مسجّلة لسه.</p>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {devices.map((d) => {
          ...
          return (
            <div key={d.deviceId} style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: 8, padding: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
                <div>
                  ...
                  {d.receiptImage && (
                    <a href={d.receiptImage} target="_blank" rel="noreferrer" style={{ fontSize: 12, marginRight: 8, color: '#0d6efd', fontWeight: 'bold' }}>
                      📎 عرض إيصال الدفع
                    </a>
                  )}
                </div>
                <div style={{ fontSize: 11, color: '#999', fontFamily: 'monospace' }} title={d.deviceId}>
  ...
  return (
    <div style={{ background: '#fff', border: '1px solid #e0e0e0', borderRadius: 8, padding: 16, marginBottom: 16 }}>
      ...
      <form onSubmit={handleChangeCredentials} style={{ maxWidth: 340 }}>
        ...
        <input type="password" placeholder="كلمة السر الحالية" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} style={inputStyle()} autoComplete="current-password" />
        <input type="password" placeholder="كلمة السر الجديدة (سيبها فاضية لو مش عايز تغيّرها)" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} style={inputStyle()} autoComplete="new-password" />
        <input type="password" placeholder="تأكيد كلمة السر الجديدة" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} style={inputStyle()} autoComplete="new-password" />
